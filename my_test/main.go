...
type Magnification struct {
	MagnificationValue float64 // 倍率值
	TriggerTimes       float64 // 触发次数
	Name               string  // 伤害名称
	IncreasedDamage    float64 // 指定增伤,存在，使用这个增伤+基础增伤
}
...
func main() {
	...
	for key, _ := range MainArticleMap {
		...
		for MainArticleMap[key] < initialization.Condition.MainArticle {
			...
			// 打印日志
			fmt.Printf("输出总伤害: %.6f万\n", NewDamage/10000)
			fmt.Printf("伤害差距: %.2f%%\n", DecimalToPercentage(NewDamage, OldDamage))
			// 更新旧伤害值
			OldDamage = NewDamage
			// 递增词条数量
			MainArticleMap[key]++
...
func (i *Initialization) HandleBasicCritical(key string, count int) {
	if key == Critical {
		...
		if critical > i.Condition.Critical {
			...
			// 打印调试信息
			fmt.Printf("暴击溢出处理: 原始暴击率=%.2f%%, 阈值=%.2f%%, 转换词条数=%d, 最终暴击率=%.2f%%, 爆伤增加=%.2f%%\n",
...
// DefenseArea 计算防御区
func (i *Initialization) DefenseArea() {
	//防御区 = 角色基数/(总防御力*(1-穿透率)*(1-破防百分比)-穿透值+角色基数)
	//角色基数：794,总防御力：873
	characterBase, TotalDefense := 793.783, 873.1613
	i.Output.DefenseArea = characterBase / (TotalDefense*(1-i.Defense.Penetration)*(1-i.Defense.DefenseBreak) - i.Defense.PenetrationValue + characterBase)
}

// ReductionResistanceArea 减抗区
func (i *Initialization) ReductionResistanceArea() {
	i.Output.ReductionResistanceArea = 1 + (i.CurrentPanel.ReductionResistance)/100
}
...
func NewInitialization() *Initialization {
	magnifications := []*Magnification{
		&Magnification{
			MagnificationValue: 65.7 + 124.9 + 160.4 + 254.4 + 309.6 + 360.6,
			TriggerTimes:       6,
			Name:               "普攻",
		},
		&Magnification{
			MagnificationValue: 415.8,
			TriggerTimes:       12,
			Name:               "落雷-追加攻击",
			IncreasedDamage:    25 + 15,
		},
		&Magnification{
			MagnificationValue: 760.2,
			TriggerTimes:       1,
			Name:               "强化特殊技",
		},
		&Magnification{
			MagnificationValue: 1128.1 + 3442.1,
			TriggerTimes:       1,
			Name:               "连携+终结技",
		},
	}
	return &Initialization{
		...
		Basic: &Basic{
			...
			BasicCritical:            19.4 + 10 + 12 + 24, // 基础暴击（角色+武器+2件套+4号位）
			BasicExplosiveInjury:     50 + 48 + 60 + 25,   // 基础爆伤（角色+武器+2件套+4号位）
			BasicIncreasedDamage:     20 + 30,             // 基础增伤（角色+武器+驱动盘）
		...
		Gain: &Gain{
			...
			AttackInternalPercentage: 24,         // 局内攻击力(武器，驱动盘绿字攻击力)
			Critical:                 0,          // 增加暴击（角色+武器+2件套+4号位）
			ExplosiveInjury:          25,         // 增加爆伤（角色+武器+2件套+4号位）
			IncreasedDamage:          24 + 25,    // 增伤（队友百分比）
